...
def read_attr(filename):
    ...
    for i in df.index.values:  # Gets the index of the row number and traverses it
        if int(df.number[i])==1 : df.number[i] = 0
        elif int(df.number[i])>1 : df.number[i] = 1
        text[df.Image[i]] = 128*df.bilateral[i]+64*df.number[i]+32*df.upper_left[i]+16*df.middle_left[i]+8*df.lower_left[i]+4*df.upper_right[i]+2*df.middle_right[i]+1*df.lower_right[i]
