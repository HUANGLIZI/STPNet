...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        image, label, text, text_tensor = sample['image'], sample['label'], sample['text'], sample['text_tensor']
        ...
        sample = {'image': image, 'label': label, 'text': text, 'text_tensor': text_tensor}
        return sample
...
class ECA(Dataset):
    def __init__(self, dataset_path: str, joint_transform=None):
        self.joint_transform = joint_transform  # using transform in torch!
        self.dataset_path = dataset_path
        ...
        # else:
        #     to_tensor = T.ToTensor()
        #     self.joint_transform = lambda x, y: (torch.Tensor(x), torch.Tensor(y))
    ...
    def __getitem__(self, idx):
        ...
        image = sitk.ReadImage(os.path.join(self.input_path, image_filename))
        mask = sitk.ReadImage(os.path.join(self.output_path, image_filename))
        image = sitk.GetArrayFromImage(image)
        mask = sitk.GetArrayFromImage(mask)/1.0       
        image = torch.Tensor(image)
        mask = torch.Tensor(mask)
        # image = image.resize_(60,112,112)
        # mask = mask.resize_(60,112,112)
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image, mask = correct_dims(image, mask)
        text = self.rowtext[mask_filename]
        text_tensor = text*np.ones((1,224,224))
        ...
        sample = {'image': image, 'label': mask, 'text': text, 'text_tensor': text_tensor}
